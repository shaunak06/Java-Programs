...
public class SimplifiedSudoku extends Applet implements Runnable
{
  ...
  protected void createModel()
  {
    ...
    try{
      ...
      {
        StringTokenizer theCells = new StringTokenizer(line, ", ");
...
//    }
  ...
  protected void updateView()
  {
      ...
      for( int col = 0; col < 16; col++ )
    {
      ...
      if(model[row][col]==11)
      {
        view[row][col].setBackground(Color.WHITE);
        view[row][col].setForeground(Color.BLACK);
      }
      if(model[row][col]==12)
      {
        Color c = new Color(0,178,238);
        view[row][col].setBackground(c);
        view[row][col].setForeground(Color.BLACK);
      }
      if(model[row][col]==13)
      {
        Color c = new Color(139,90,0);
        view[row][col].setBackground(c);
        view[row][col].setForeground(Color.BLACK);
      }
      if(model[row][col]==14)
      ...
      if(model[row][col]==15)
      {
        Color c = new Color(56,142,142);
        view[row][col].setBackground(c);
        view[row][col].setForeground(Color.BLACK);
      }
      if(model[row][col]==16)
      {
        Color c = new Color(238,44,44);
        view[row][col].setBackground(c);
        view[row][col].setForeground(Color.BLACK);
      }
